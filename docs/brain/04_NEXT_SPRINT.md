...
> **Актуализация 2026-09-22 (аудит).** Alembic head в репозитории —
> `0058_task_assignee_and_standalone` (в проде накатан `0057`; `0058` лежит в
> цепочке `0056 → 0058 → 0057`, поэтому по именам файлов она читается не по
> порядку). Спринт «Руководитель отдела продаж» ниже фактически закрыт и в
> проде — чекбоксы остались непротиснутыми, и по протоколу «взять первый
> `- [ ]`» можно переделать уже сделанное. Артефакты аудита (ARCH-01..05,
> TASK-01..03, DEV-01/02, SEC-D0..D4) лежат вне репозитория, в scratchpad
> программы; сюда перенесены только правки кода и документации.

---

## Sprint — Руководитель отдела продаж (раздача базы + задачи команде)

Роль `head` («Руководитель») уже была в коде: панель команды, формы,
автоматизации, передача чужих лидов. Не хватало двух вещей — раздать базу
конкретному менеджеру и поставить ему задачу. Спринт достраивает обе.

**G1 — выдача лидов из базы (бэкенд)**
- [ ] `POST /leads/assign` (admin/head): режим списка id и режим «N по фильтру
      из пула». Перехват чужой карточки сохраняет прежнего владельца в
      `transferred_from`; фильтр-режим пул не покидает.
- [ ] Одно уведомление `leads_assigned` на пачку (снято часовое окно
      дедупликации), запись `lead.assign_batch` в журнал, `lead_assigned` в ленту лида.

**G2 — выдача лидов (интерфейс)**
- [ ] `/leads-pool`: выделение строк галочками + панель «Выдать менеджеру».
- [ ] Модалка выдачи: выбор менеджера, режим «выделенные» / «N по фильтру».
- [ ] «Назначить» в карточке лида для руководителя.

**G3 — задачи с исполнителем (бэкенд)**
- [ ] Миграция `0058`: `activities.assignee_user_id`, `activities.workspace_id`
      (backfill из `leads`), `activities.lead_id` → nullable (задача без лида).
- [ ] `/me/tasks` отдаёт и назначенное лично, не только задачи по своим лидам.
- [ ] `GET /tasks` для руководителя: фильтр по исполнителю и статусу.

**G4 — задачи (интерфейс)**
- [ ] `/tasks`: вкладки «Мои» / «Поставлено мной» / «Команда» (для head).
- [ ] «Новая задача» с выбором исполнителя и необязательным лидом.
...
## Ad hoc — алерт «не работает с лидами» на панели менеджеров

- [x] Починить дату «был активен»: брать самый свежий сигнал (вход / действие
      по лиду), а не первый непустой. Старый `or` выбрасывал дату входа, если
      у менеджера было хоть одно действие, — 21.07.2026 панель писала
      «Кирилл не заходил 12 дней», хотя он был в CRM 20 минут назад.
- [ ] Вернуть потерянный полезный сигнал отдельным типом алерта: «N дней не
      трогает лидов» (считается по `max(activities.created_at)`, порог — те же
      48 часов). Сейчас после фикса эта информация из плашек ушла и видна
      только как «Действий всего: 0» на карточке.
      Место: `alerts` в `app/company/services.py::managers`, отрисовка —
      `alertText` в `components/today/TeamOverview.tsx`.
