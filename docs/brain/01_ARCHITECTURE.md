...
> ⚠️ **Этот документ описывает целевую/плановую архитектуру (Phase 1 MVP) и местами
> устарел** (напр. хостинг Vercel/Railway → bare-metal; доменов в коде сейчас 25, а не ~16).
> Актуальная карта *фактического* функционала — по доменам, с механиками и подводными
> камнями — лежит в [`docs/features/`](../features/README.md). Руководство для
> менеджера — [`docs/manual/РУКОВОДСТВО-МЕНЕДЖЕРА.md`](../manual/РУКОВОДСТВО-МЕНЕДЖЕРА.md).
