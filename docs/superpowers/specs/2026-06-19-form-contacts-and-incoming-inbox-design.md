...
3. `extract_person_name(payload) -> str | None`
   ...
   - else `None`.
4. `extract_question(payload, *, limit=200) -> str` — replaces `extract_snippet`:
   ...
   - If the value is a `Label: value` blob, return the value of its
     `сообщение/вопрос/message/comment` label when present; else the whole value.
   - Trim to `limit`.
...
- `list_inbox` item dict: replace `snippet` with `question` (via
  `extract_question`) and add `contact_name` (via `extract_person_name`).
  `source_domain` is already in the dict.
- `InboxItemOut`: add `contact_name: str | None` and `question: str | None`;
  ensure `source_domain: str | None` is present. Drop `snippet`.
- `extract_snippet` is renamed/replaced by `extract_question`; update its export
  and the `count_new`/`list_inbox` callers.

### Frontend — `apps/web/app/(app)/incoming/page.tsx` + `lib/types.ts`

`InboxItemOut` type: add `contact_name`, `question`; drop `snippet`. The `Row`
component is restructured:
...
- **Вопрос**: a labelled block ("Вопрос клиента") showing `question`,
  `line-clamp-2`, instead of the all-fields blob.
...
- `extract_question`: clean field; blob with `Сообщение:` line; capitalized key
  (`Сообщение`) regression; absent → "".
- `create_lead_from_submission`: email+phone → Contact created and set as primary
  ЛПР; neither → no Contact; existing `primary_contact_id` is not overwritten.
...
- Exact `raw_payload` shape is confirmed against one real record at the start of
  implementation (no endpoint exposes it; inspect via a read-only DB query or a
  temporary debug log, removed before commit). The extraction is designed to be
  robust to both clean-field and `Label: value`-blob payloads regardless.
