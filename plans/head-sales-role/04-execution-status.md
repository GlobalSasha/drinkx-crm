...
| C0 | in_progress | оркестратор | `apps/web/lib/types.ts`, `lib/tasks.ts`, `lib/hooks/use-tasks.ts` | `npx tsc --noEmit` = 4 известные ошибки | `mode`/`only_pool` в типах, `keepPreviousData`, `enabled` |
| W0 | pending | fast-worker | `components/ui/UserSelect.tsx`, `lib/api-error.ts` + тесты | `vitest run`, `tsc` | после C0 |
| F0 | pending | fast-worker | `use-my-tasks.ts`, `TaskTable.tsx`, `TaskReminders.tsx` | `tsc` → 3 ошибки только в `tasks/page.tsx` | после C0 |
