...
### Безопасность
- **[открыто] Хардкод тест-кредов.** `apps/web/app/sign-in/page.tsx` — кнопка
  «🧪 Войти как тестовый пользователь» логинит `test@drinkx.tech` с паролем,
  **зашитым в клиентский код**. В проде это открытый вход под тест-аккаунтом —
  гейтить по env (только dev) или убрать. *(эта сессия)*
