...
const TOC: { id: string; label: string; icon: LucideIcon }[] = [
  ...
  { id: "team", label: "Команда (для руководителей)", icon: BarChart3 },
  { id: "leadcard", label: "Карточка лида", icon: Target },
  { id: "quote", label: "Коммерческие предложения (КП)", icon: ReceiptText },
  { id: "duplicates", label: "Дубли лидов", icon: GitMerge },
  { id: "pool", label: "База лидов (пул)", icon: Target },
  { id: "incoming", label: "Входящие заявки", icon: Inbox },
...
const GLOSSARY: { term: string; def: string }[] = [
  ...
  { term: "КП", def: "Коммерческое предложение: позиции, скидки, НДС и итог. Собирается в карточке лида, печатается в PDF из браузера." },
  { term: "Дубль", def: "Второй лид той же компании. CRM находит похожих по домену почты, телефону или названию — их можно объединить в один." },
  { term: "Входящая заявка", def: "Обращение клиента через форму на сайте. Попадает в раздел «Входящие заявки» и создаёт лид с контактом." },
  { term: "UTM-источник", def: "Откуда пришёл лид (utm_source из ссылки веб-формы). Виден в карточке лида, а сводка по каналам — в «Прогнозе»." },
];
...
const FAQ: { q: string; a: string }[] = [
  ...
  {
    q: "Как собрать КП и сохранить его в PDF?",
    a: "Откройте карточку лида → вкладка «КП» → «Новый КП». Добавьте позиции из каталога или вручную, проставьте скидки и НДС — итог посчитается сам. Нажмите «Печать / PDF»: откроется чистый лист, дальше Ctrl/⌘+P → «Сохранить как PDF». CRM сам клиенту ничего не отправляет — готовый PDF вы отправляете сами.",
  },
  {
    q: "Где заявки, отправленные через форму на сайте?",
    a: "В сайдбаре — раздел «Входящие заявки». Там лента всех обращений с веб-форм: контакт, телефон/почта (в один клик копируются), вопрос клиента и статус. Клик по заявке открывает карточку соответствующего лида.",
  },
  {
    q: "У клиента два лида — как объединить?",
    a: "Откройте карточку лида → меню «⋯» → «Найти дубли». CRM покажет похожих по домену почты, телефону или названию. Отметьте нужных галочками → «Объединить» → подтвердите. Текущий лид станет основным, дубликаты уедут в архив (обратимо), а их история и контакты переедут в основной.",
  },
  {
    q: "Где посмотреть, на каком этапе застревают сделки?",
    a: "В разделе «Прогноз» внизу есть таблица «Где застревают сделки»: по каждому этапу видно медианное время и сколько сделок стоят дольше нормы. Рядом — «Каналы привлечения»: сколько лидов и выручки принёс каждый UTM-источник.",
  },
];
    ...
    <div className={pageContainerVariants({ surface: "reading" })}>
      ...
      <div className="grid lg:grid-cols-[220px_1fr] gap-8">
        ...
        <div className="min-w-0">
          ...
          <Section id="forecast" icon={TrendingUp} kicker="Прогноз" title="Прогноз продаж">
            ...
            <Card title="Что ниже">
              <KV items={[
                ...
                ["Каналы привлечения", "Таблица по UTM-источникам: сколько сделок, конверсия и выручка по каждому каналу. Лиды без меток идут как «Прямые / без UTM»."],
                ["Где застревают сделки", "По каждому этапу: сколько сделок прошло, медианное время на этапе и сколько «застряли» дольше нормы. Видно узкие места воронки."],
          ...
          {/* КОМАНДА */}
          <Section id="team" icon={BarChart3} kicker="Для руководителей" title="Команда">
            <Alert kind="info">
              Раздел <strong>«Команда»</strong> в сайдбаре виден только руководителям и админам
              (admin / head). У менеджера он ведёт в настройки — это нормально для роли.
            </Alert>
            <p className="type-body text-brand-muted-strong mb-3">
              Две вкладки-переключателя в правом верхнем углу:
            </p>
            <div className="grid sm:grid-cols-2 gap-3 mb-3">
              <Card title="Manager's Dashboard" icon={BarChart3}>
                <p className="type-body text-brand-muted-strong">
                  Сводная таблица загрузки команды: сколько у каждого активных лидов и как они
                  распределены по этапам воронки. Сразу видно, кто перегружен, а у кого пусто.
                </p>
              </Card>
              <Card title="Активность" icon={TrendingUp}>
                <p className="type-body text-brand-muted-strong">
                  Карточки по каждому менеджеру за период (Сегодня / Неделя / Месяц): отправлено
                  <strong> КП</strong>, взято <strong>из пула</strong>, <strong>продвинуто</strong>
                  лидов, выполнено <strong>задач</strong>, и когда был активен в последний раз.
                </p>
              </Card>
            </div>
            <Card title="Портфель менеджера">
              <p className="type-body text-brand-muted-strong">
                Клик по карточке менеджера открывает его портфель сделок: суммы и количество по
                сегментам (Ритейл, HoReCa, QSR, АЗС…) и этапам. Удобно на разборе пайплайна один на один.
              </p>
            </Card>
          </Section>
          ...
          <Section id="leadcard" icon={Target} kicker="Лиды" title="Карточка лида">
            ...
            <p className="type-caption text-brand-muted mb-3">Семь вкладок:</p>
            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              ...
              <Card title="Информация" icon={Sparkles}><p className="type-body text-brand-muted-strong">Справа — блок <strong>«Параметры сделки»</strong>: редактируемые поля (Сумма, Количество, Оборудование, Тип, Приоритет, Сегмент, Город, ИНН, Сайт, Email, Телефон). Клик по строке — правка inline, Enter — сохранить, Esc — отменить. Слева — AI-бриф.</p></Card>
              ...
              <Card title="КП" icon={ReceiptText}><p className="type-body text-brand-muted-strong">Коммерческие предложения по лиду: список и сборщик с печатью в PDF. Подробно — в разделе <a href="#quote" className="text-brand-accent hover:underline">«Коммерческие предложения»</a>.</p></Card>
            ...
            <Card title="Что можно делать">
              <Steps items={[
                ...
                ["Собрать КП", "Вкладка «КП» — позиции, скидки, НДС, печать в PDF. Подробно — в разделе «Коммерческие предложения»."],
                ["Найти дубли", "Меню «⋯» → «Найти дубли»: объединить второй лид той же компании в текущий. Подробно — в разделе «Дубли лидов»."],
          ...
          {/* КП */}
          <Section id="quote" icon={ReceiptText} kicker="Продажи" title="Коммерческие предложения (КП)">
            <p className="type-body text-brand-muted-strong mb-3">
              КП собирается прямо в карточке лида — вкладка <strong>«КП»</strong>. Можно вести
              несколько предложений по одному клиенту, печатать их в PDF и переносить итог в сумму сделки.
            </p>
            <Alert kind="info">
              Перед первым КП загрузите каталог: <strong>Настройки → «Каталог КП»</strong> — товары и
              цены, которые потом подставляются в позиции. Можно и без каталога — добавляя строки вручную.
            </Alert>
            <Card title="Как собрать предложение">
              <Steps items={[
                ["Открыть вкладку «КП»", "Карточка лида → «КП». Список предложений по этому клиенту + кнопка «Новый КП»."],
                ["Добавить позиции", "«+ из каталога» подставит название и цену; «+ позиция» — свободная строка. Укажите количество, цену и скидку по строке."],
                ["Скидка и НДС", "Скидку на всё КП — в рублях; ставку НДС — 20%, без НДС или свою. Итоги (сумма позиций, скидка, НДС, ИТОГО) считаются автоматически."],
                ["Статус", "Черновик → «Отметить отправленным» → «Принято» / «Отклонено». Пока черновик — его можно править и удалить; после отправки позиции фиксируются."],
                ["Печать / PDF", "Кнопка «Печать / PDF» откроет чистый лист предложения → Ctrl/⌘+P → «Сохранить как PDF»."],
                ["Сумма сделки = итог", "Кнопка переносит ИТОГО предложения в поле «Сумма» сделки (в «Параметрах сделки»)."],
              ]} />
            </Card>
            <Alert kind="warn">
              CRM не отправляет КП клиенту сам — это намеренно. Менеджер сохраняет PDF и отправляет
              его клиенту привычным способом (почта, мессенджер).
            </Alert>
          </Section>

          {/* ДУБЛИ */}
          <Section id="duplicates" icon={GitMerge} kicker="Качество данных" title="Дубли лидов">
            <p className="type-body text-brand-muted-strong mb-3">
              Если у одной компании завелось два лида, их можно объединить в один — без потери истории.
              Запускается из карточки лида: меню <strong>«⋯» → «Найти дубли»</strong>.
            </p>
            <Card title="Как объединить">
              <Steps items={[
                ["Открыть «Найти дубли»", "Карточка лида → «⋯» → «Найти дубли». Текущий лид всегда станет основным."],
                ["Проверить кандидатов", "CRM покажет похожих по домену почты, телефону или названию компании — с почтой, телефоном и городом для сверки."],
                ["Отметить нужных", "Галочками выберите те лиды, что действительно дубликаты текущего."],
                ["Объединить и подтвердить", "«Объединить» → шаг подтверждения. Дубликаты уйдут в архив (обратимо), а их история и контакты переедут в основной лид."],
              ]} />
            </Card>
            <Alert kind="info">
              Склейка никогда не происходит автоматически — только по вашему явному выбору. На
              объединённом лиде появляется пометка «поглотил дубликаты».
            </Alert>
          </Section>
          ...
          {/* ВХОДЯЩИЕ ЗАЯВКИ */}
          <Section id="incoming" icon={Inbox} kicker="Новые клиенты" title="Входящие заявки с сайта">
            <p className="type-body text-brand-muted-strong mb-3">
              Раздел <strong>«Входящие заявки»</strong> в сайдбаре — лента обращений, отправленных
              через формы на ваших сайтах. Каждая заявка создаёт лид с контактом.
            </p>
            <Card title="Что в ленте">
              <KV items={[
                ["Кто обратился", "Имя контакта и компания. Телефон и email кликабельны (позвонить / написать) и копируются одной кнопкой."],
                ["Вопрос клиента", "Текст из формы — что именно спрашивает клиент, или краткая сводка заявки."],
                ["Источник и статус", "Канал и домен сайта; статус заявки: «Новая», «Назначен · имя», «В пуле» или «Просмотрена»."],
                ["Открыть лида", "Клик по заявке открывает карточку соответствующего лида — работайте как обычно."],
              ]} />
            </Card>
            <Card title="Фильтры и прочтение" className="mt-2">
              <KV items={[
                ["По форме", "Чипы вверху — показать заявки только из конкретной формы."],
                ["Только новые", "Оставить непросмотренные."],
                ["Отметить прочитанными", "Кнопка вверху обнуляет счётчик новых — как «прочитать всё» в почте."],
              ]} />
            </Card>
            <Alert kind="tip">
              Настройку самих форм и авто-ответа клиенту (свой под каждую форму) делают руководители
              в разделе «Формы».
            </Alert>
          </Section>
