...
const TOC: { id: string; label: string; icon: LucideIcon }[] = [
  ...
  { id: "tasks", label: "Задачи и напоминания", icon: CheckSquare },
...
const GLOSSARY: { term: string; def: string }[] = [
  ...
  { term: "Fit-score", def: "Оценка AI (0–10): насколько лид подходит под идеального клиента." },
  { term: "Скоринг", def: "Оценка лида по критериям (0–100). Влияет на приоритет в плане дня." },
...
const FAQ: { q: string; a: string }[] = [
  ...
  {
    q: "Список задач на сегодня кажется устаревшим",
    a: "Нажмите «Перегенерировать план» на экране «Сегодня». Система пересчитает приоритеты по актуальным данным за несколько секунд.",
  },
  {
    q: "AI-бриф пустой или обогащение «крутится»",
    a: "Обогащение идёт в фоне и занимает несколько секунд. Если зависло надолго — обновите страницу и запустите обогащение ещё раз из карточки лида.",
  },
  {
  ...
  },
  {
    ...
    a: "Блейк — помощник, а не автопилот. Финальное решение всегда за вами; используйте подсказки как отправную точку и проверяйте их в контексте конкретной сделки.",
  },
  {
        ...
        <div className="min-w-0">
          ...
          <Section id="start" icon={HelpCircle} kicker="Добро пожаловать" title="Ключевые понятия">
            ...
            <Card title="Типичное утро менеджера" icon={CalendarDays} className="mt-4">
              <Steps
                items={[
                  ["Открыть «Сегодня»", "Главная страница после входа — всё уже готово."],
                  ["Пройтись по задачам сверху вниз", "Система отсортировала их по важности."],
                  ["Отметить выполненное галочкой", "Прямо в виджете, не заходя в карточку."],
                  ["Заглянуть в «Подвисшие лиды»", "Чтобы никого не потерять из виду."],
                ]}
              />
            </Card>
            <Alert kind="info">
          ...
          <Section id="today" icon={CalendarDays} kicker="Ежедневная работа" title="Рабочий стол «Сегодня»">
            ...
            <Alert kind="success">
              <strong>Список задач формируется автоматически каждое утро</strong> — система ранжирует
              лидов по срочности, сумме сделки и приоритету. Если план устарел — перегенерируйте.
            </Alert>
            <Card>
              <KV items={[
                ["Задачи на сегодня", "Список с чекбоксами и прогрессом X/Y. Отмечайте выполненное прямо здесь."],
                ["Follow-ups", "Напоминания со сроком: звонки, отправка материалов."],
                ["Подвисшие лиды", "Клиенты без движения — займитесь ими в первую очередь."],
                ["Мини-воронка", "Сводка: сколько лидов на каждом этапе."],
                ["Фокус-лиды", "Приоритетные сделки на сегодня."],
                ["Инсайты Блейка", "Короткие AI-подсказки по активным сделкам."],
              ]} />
            </Card>
          </Section>
          ...
          <Section id="leadcard" icon={Target} kicker="Лиды" title="Карточка лида">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              ...
              <Card title="Лента активности" icon={MessageCircle}><p className="type-body text-brand-muted-strong">Комментарии, задачи, письма, звонки, изменения этапа, подсказки Блейка.</p></Card>
              <Card title="Сделка и AI-бриф" icon={Sparkles}><p className="type-body text-brand-muted-strong">Сумма, оборудование, количество + автоматически собранные данные о компании.</p></Card>
              <Card title="Скоринг и история" icon={Target}><p className="type-body text-brand-muted-strong">Оценка по критериям (0–100) и время на каждом этапе.</p></Card>
            </div>
            <Card title="Что можно делать">
              <Steps items={[
                ["Добавить комментарий, задачу или напоминание", "Всё попадает в единую ленту активности."],
                ["Перевести лид на другой этап", "Система проверит гейты и зафиксирует переход."],
                ["Запустить AI-обогащение", "Соберёт данные о компании из открытых источников. В фоне, несколько секунд."],
                ["Открыть чат с Блейком", "Кнопка-кружок внизу — совет по сделке, подготовка к звонку."],
              ]} />
            </Card>
          </Section>
          ...
          <Section id="blake" icon={Bot} kicker="AI-помощник" title="Блейк — ваш AI-помощник">
            <p className="type-body text-brand-muted-strong mb-3">
              AI-коуч по продажам. Знает контекст сделки: этап, контакты, AI-бриф, базу знаний DrinkX.
            </p>
            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              <Card title="Подсказка-баннер" icon={Lightbulb} className="border-l-4 !border-l-brand-accent">
                <p className="type-body text-brand-muted-strong">Короткая рекомендация в карточке лида. Если Блейк не уверен — не предложит действия.</p>
              </Card>
              <Card title="Чат" icon={MessageCircle} className="border-l-4 !border-l-brand-accent">
                <p className="type-body text-brand-muted-strong">Кнопка-кружок «Блейк». Подготовка к встрече, работа с возражениями, черновик КП.</p>
              </Card>
            </div>
            ...
            <Alert kind="tip">
              <strong>Важно:</strong> Блейк советует — решение всегда за вами. Он ничего не отправляет
              клиенту сам: никаких автоматических писем без вашего подтверждения.
            </Alert>
          </Section>

          {/* ЗАДАЧИ */}
          <Section id="tasks" icon={CheckSquare} kicker="Планирование" title="Задачи и напоминания">
            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              <Card title="Задачи дня" icon={CheckSquare}><p className="type-body text-brand-muted-strong">Что система предложила на сегодня — виджет на «Сегодня».</p></Card>
              <Card title="Follow-ups" icon={Bell}><p className="type-body text-brand-muted-strong">Шаги по конкретному лиду. Новый лид получает стартовый чек-лист автоматически.</p></Card>
            </div>
            <Card title="Стартовый чек-лист нового лида">
              <Steps items={[
                ["Первичный контакт", "Позвонить или написать, представиться, узнать о потребности."],
                ["Discovery-звонок", "Квалифицировать лид, выяснить боли и бюджет."],
                ["Отправить материалы", "Презентация, КП или кейсы."],
              ]} />
            </Card>
            <Alert kind="info">
              За <strong>24 часа до срока</strong> follow-up система пришлёт уведомление и добавит
              напоминание в ленту лида. Отметьте выполненным, когда сделаете.
            </Alert>
          </Section>
          ...
          <Section id="notifications" icon={Bell} kicker="Система" title="Уведомления">
            <Card title="Что приходит" className="mb-3">
              <ul className="space-y-1.5 type-body text-brand-muted-strong list-none">
                ...
                <li>⏰ Приближение срока follow-up (за 24 ч)</li>
                <li>📅 Готовность плана на день</li>
              </ul>
            </Card>
            <Card>
              <KV items={[
          ...
          <Section id="profile" icon={Settings} kicker="Профиль" title="Личные настройки">
            <Card>
              <KV items={[
                ["Имя и фото", "Отображается в ленте активности и при передаче лидов."],
                ["Часовой пояс", "Влияет на расписание уведомлений и план дня."],
                ["Рабочие часы", "Система распределяет задачи в плане с учётом графика."],
                ["Специализация", "Сегменты, с которыми работаете — влияет на релевантность рекомендаций."],
