...
const FAQ: Record<View, [string, string][]> = {
  manager: [
    [
      "Не даёт перевести клиента на следующий этап",
      "Не хватает обязательного. Чаще всего это контакт с ролью «Экономический покупатель» — тот, кто решает про деньги. Откройте карточку → вкладку «Контакты» → добавьте такого человека и попробуйте снова. Если система просит причину, а не запрещает — условие мягкое: объясните, почему пропускаете, и она пропустит.",
    ],
    [
    ...
    [
      "Что значит «N дней на этапе»? Я только вчера взял клиента",
      "Это сколько дней клиент стоит на этом этапе — независимо от того, кто им занимается. Он мог неделю лежать в базе, счётчик всё это время шёл. Сколько работаете с ним лично вы — видно по дате «В работе с …».",
    ],
    [
      "Взял не того клиента",
      "Кнопка «Вернуть в базу» в шапке карточки. Клиент снова станет доступен всей команде.",
    ],
    [
    ...
    [
      ...
      "Значит, этот адрес или номер CRM не знает. Раздел «Мессенджеры» → найдите письмо среди неразобранных и привяжите к нужному клиенту вручную. Так бывает, когда человек пишет с новой почты.",
    ],
    [
    ...
    <>
      <Section id="start" icon={Compass} kicker="Первый день" title="С чего начать">
        ...
        <Card title="Как проходит день" icon={CalendarDays}>
          <Steps items={[
            ["Открыли «Сегодня»", "Первый экран после входа. Сверху — задачи по срокам, рядом «Устаревает»: клиенты, о которых давно не было слышно."],
            ...
            ["Добрали новых", "«База лидов» → «Взять в работу». Взяли — ведёте; не пошло — вернули в базу, чтобы взял коллега."],
          ]} />
        </Card>
        <GlossaryDetails />
        <Details summary="Что где лежит в меню">
          <KV items={[
            ["Поиск (⌘K)", "Ищет компании, лиды и контакты. С трёх букв понимает опечатки: «Мкдоналдс» найдёт «Макдоналдс»."],
            ...
            ["База лидов", "Общий банк клиентов, которых пока никто не ведёт."],
            ["Мессенджеры", "Почта, Telegram, MAX, звонки. Сюда же попадают письма, которые CRM не смогла привязать к клиенту."],
      ...
      <Section id="tasks" icon={CheckSquare} kicker="Ежедневно" title="Задачи: как на них реагировать">
        ...
        <Details summary="Где ставить и где смотреть">
          <KV items={[
            ...
            ["Где увидите", "На «Сегодня», в разделе «Задачи» и внутри карточки клиента — это один и тот же список."],
      ...
      <Section id="card" icon={Target} kicker="Работа с клиентом" title="Карточка клиента">
        <p className="type-body text-brand-muted-strong mb-3">
          Открывается кликом по клиенту в воронке или в поиске. Слева — то, что нужно
          всегда: сумма, приоритет, следующий шаг, телефон и почта. Справа — шесть вкладок.
        </p>
        <Card>
          <KV items={[
        ...
        <Alert kind="info">
          Поля правятся прямо на месте: щёлкнули по строке, поменяли, Enter — сохранено.
          Кнопки «Редактировать» нет и не ищите.
        </Alert>
        ...
        <Details id="duplicates" summary="У клиента завелось два лида">
          <p className="type-body text-brand-muted-strong">
            Меню <strong>«⋯» → «Найти дубли»</strong>. CRM покажет похожих по почте,
      ...
      <Section id="stages" icon={Kanban} kicker="Воронка" title="Как двигать сделку">
        ...
        <Alert kind="warn">
          На некоторых этапах система вас остановит, пока не собрано главное. Чаще
          всего это <strong>«Multi-stakeholder»</strong>: туда не пустят, пока среди
          контактов нет человека с ролью «Экономический покупатель» — того, кто
          решает про деньги. Добавьте его во вкладке «Контакты» и попробуйте снова.
        </Alert>
        <Details summary="Все этапы и что делать, если не пускает">
          ...
          <KV items={[
            ["Не пускает совсем", "Не хватает обязательного — чаще всего контакта с ролью «Экономический покупатель»."],
            ["Просит причину", "Условие мягкое: объясните, почему пропускаете, и система пропустит. Причина сохранится в истории."],
      ...
      <Section id="incoming" icon={Inbox} kicker="Новые клиенты" title="Где брать клиентов">
        <div className="grid sm:grid-cols-2 gap-3">
          <Card title="База лидов" icon={Target}>
            <p className="type-body text-brand-muted-strong">
              Компании, которых пока никто не ведёт. Отфильтруйте по городу и сегменту,
              нажмите <strong>«Взять в работу»</strong> — клиент ваш. Не пошло —
              «Вернуть в базу», и его возьмёт коллега.
            </p>
          </Card>
