...
async def run_extract_and_match(*, db: AsyncSession, job_id: uuid.UUID) -> None:
    ...
    # Decorate with any extraction failures, then clear staging
    if extract_failures:
        stats["extract_failures"] = extract_failures
    _clear_staged_files(job)
    # Merge runtime stats into job.stats_json (keep any existing keys other than _staged_files)
    merged = {**(job.stats_json or {}), **stats}
    job.stats_json = merged
