...
async def effective_export_selection(
    ...
    if is_privileged(actor):
        # Значение статуса — из известного набора: иначе фильтр молча дал бы
        # пустую выгрузку (`app/leads/selection.py::ASSIGNMENT_STATUSES`).
        selection.validate_assignment_status()
        # Дальше проверяется только одно: получатель фильтра
    ...
    # Политика доступа раньше формата: менеджеру любой статус, кроме
    # `assigned`, закрыт — неизвестный в том числе, и ответ на него остаётся
    # прежним 403, а не 422.
