...
Ищи людей в текстах источников. Если видишь имя + должность в любом
тексте (вакансии HH, сайт команды, статьи, LinkedIn, пресс-релизы) —
добавляй в contacts_found. Если источники совсем не содержат имён —
верни пустой массив.
...
Для крупных ритейлеров (Пятёрочка, Перекрёсток, Магнит, Лента, Дикси,
Ашан, Метро) ищи директора по закупкам или категорийного менеджера в
HH.ru вакансиях — там часто указывают, кому подчиняется роль («подчинение
напрямую N.N.», «руководитель направления — …»).

Поля объекта: name, title, email, phone, linkedin_url, source,
confidence (число 0.0–1.0). confidence — обязательное поле: 0.8+ если
имя+должность напрямую упомянуты в источнике, 0.5–0.7 если выводится
по контексту, 0.3–0.4 если предположение.
